...
export function getActionTypeText(actionType: string) {
  const map: Record<string, string> = {
    ...
    PLAN_GENERATED: "每日计划生成",
    PLAN_SCHEDULED: "计划执行入队",
    PLAN_DELAYED: "计划延后执行",
    PLAN_EXECUTE_PRECHECKED: "计划执行结果",
    PLAN_EXECUTE_BLOCKED: "计划预检拦截",
    PLAN_EXECUTE_FAILED: "计划执行失败",
    PLAN_EXECUTE_REQUEUED: "计划重新入队",
    PLAN_REQUEUED: "计划重新入队",
    PLAN_APPROVED: "计划审核通过",
    PLAN_REJECTED: "计划审核驳回",
    PLAN_STOPPED: "计划已停止",
    PLAN_STATUS_UPDATED: "计划状态更新",
    PLAN_DELETED: "计划已删除",
    FIRST_COMMENT_EXECUTE_SUCCESS: "首评执行成功",
    FIRST_COMMENT_EXECUTE_FAILED: "首评执行失败",
    FIRST_COMMENT_REQUEUED: "首评顺延重试",
    AUTO_CHECKIN_DAILY_RUN: "签到自动执行",
    AUTO_FIRST_COMMENT_DAILY_RUN: "首评自动执行",
    ...
    INTERACTION_APPROVED: "互动任务通过",
    INTERACTION_REJECTED: "互动任务驳回",
    INTERACTION_DELAYED: "互动任务延后执行",
    INTERACTION_SCHEDULED: "互动任务执行入队",
    INTERACTION_STOPPED: "互动任务已停止",
    INTERACTION_EXECUTE_PRECHECKED: "互动任务执行结果",
    INTERACTION_EXECUTE_BLOCKED: "互动任务预检拦截",
    ACTION_JOB_SCHEDULED: "编排任务入队",
    ACTION_JOB_STOPPED: "编排任务停止",
    ACTION_JOB_STEP_SUCCESS: "编排分步成功",
    ACTION_JOB_STEP_FAILED: "编排分步失败",
