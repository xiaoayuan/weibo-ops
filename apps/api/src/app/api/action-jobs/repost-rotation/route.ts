...
import { startRepostRotationJobSchema } from "@/src/lib/validators";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const copywritingTexts = parsed.data.copywritingTexts && parsed.data.copywritingTexts.length > 0 ? parsed.data.copywritingTexts : fallbackTexts;
