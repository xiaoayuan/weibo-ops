import { deleteProxyNode, reassignAccountsForProxyNode, updateProxyNode } from "@/src/lib/proxy-pool";
import { requireApiRole } from "@/src/lib/permissions";
import { updateProxyNodeSchema } from "@/src/lib/validators";
...
export async function DELETE(_request: Request, context: RouteContext<"/api/proxy-nodes/[id]">) {
  ...
  try {
    ...
    const message = result?.unboundCount && result.unboundCount > 0 ? `代理节点已删除，${result.unboundCount} 个账号已解除代理绑定` : "代理节点已删除";
