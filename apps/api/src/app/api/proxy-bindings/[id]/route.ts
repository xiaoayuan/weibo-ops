...
import { payloadProxyBindingSchema } from "@/src/lib/validators";
...
export async function PATCH(request: Request, context: RouteContext<"/api/proxy-bindings/[id]">) {
  ...
  try {
    ...
    const parsed = payloadProxyBindingSchema.safeParse(body);
    ...
    const candidateIds = [parsed.data.proxyNodeId, parsed.data.backupProxyNodeId, parsed.data.fallbackProxyNodeId].filter(
