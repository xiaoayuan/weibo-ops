...
import { markWeiboQrLoginPersisted, pollWeiboQrLogin } from "@/src/lib/qr-login";
import { checkWeiboSession } from "@/src/lib/session-checker";
...
export async function GET(request: Request, context: RouteContext<"/api/accounts/[id]/session/qr/status">) {
  ...
  try {
    ...
    return Response.json({ success: false, message: error instanceof Error ? error.message : "查询扫码状态失败" }, { status: 500 });
  }
}
