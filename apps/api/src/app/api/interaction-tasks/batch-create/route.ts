...
import { createInteractionBatchSchema } from "@/src/lib/validators";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const availableAccounts = await prisma.weiboAccount.findMany({
      where: { id: { in: parsed.data.accountIds }, ownerUserId: auth.session.id },
      select: { id: true },
    });
    ...
    return Response.json({
      success: true,
      data: tasks,
      meta: { createdCount: createdTaskIds.length, skippedDuplicateCount, skippedTargetCount },
      message:
