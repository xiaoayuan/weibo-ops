...
export function buildAiCopywritingTags(input: AiCopywritingGenerateInput, batchId: string) {
  return ["AI生成", `业务:${businessTypeText[input.businessType]}`, `语气:${toneText[input.tone]}`, `长度:${lengthText[input.length]}`, `批次:${batchId}`];
}
...
async function requestAiText(prompt: string) {
  ...
  const response = await fetch(baseUrl, {
    ...
    body: JSON.stringify({
      ...
      temperature: 0.9,
      messages: [
        { role: "system", content: "你擅长写适合微博运营场景的真实中文短文案。" },
...
export async function generateAiCopywriting(input: AiCopywritingGenerateInput) {
  ...
  return {
    batchId: randomUUID(),
    candidates: lines.map((item, index) => ({ title: buildTitle(input, index), content: item })),
  };
}
...
export async function rewriteAiCopywriting(input: AiCopywritingGenerateInput & { sourceContent: string }) {
  ...
  return {
    batchId: randomUUID(),
    candidates: lines.map((item, index) => ({ title: `${businessTypeText[input.businessType]}-改写-${index + 1}`, content: item })),
  };
}
