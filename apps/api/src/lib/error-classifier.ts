import type { RiskRules } from "@/src/lib/risk-rules";
...
export function classifyExecutionOutcome(input: ClassifyInput, rules: RiskRules): ExecutionErrorClass {
  ...
  if (isExplicitAccountVerificationRisk(text)) return "ACCOUNT_RISK";

  if (includesAny(text, rules.keywords.accountRisk)) return "ACCOUNT_RISK";
  if (includesAny(text, rules.keywords.platformBusy)) return "PLATFORM_BUSY";
  if (includesAny(text, rules.keywords.transientNetwork)) return "TRANSIENT_NETWORK";
  if (includesAny(text, rules.keywords.targetIssue)) return "TARGET_ISSUE";
  if (includesAny(text, rules.keywords.contentIssue)) return "CONTENT_ISSUE";
