...
export function getAccountTimingDecision(accountId: string, taskLabel: string, config: AccountTimingConfig): AccountTimingResult {
  ...
  if (waitMs > 0 && jitterMs > 0) {
    return { windowEnabled: config.scheduleWindowEnabled, windowStart: config.executionWindowStart, windowEnd: config.executionWindowEnd, waitMs: waitMs + jitterMs, jitterMs, reason: "WINDOW_AND_JITTER" };
  }
  if (waitMs > 0) {
    return { windowEnabled: config.scheduleWindowEnabled, windowStart: config.executionWindowStart, windowEnd: config.executionWindowEnd, waitMs, jitterMs: 0, reason: "WINDOW_WAIT" };
  }
  if (jitterMs > 0) {
    return { windowEnabled: config.scheduleWindowEnabled, windowStart: config.executionWindowStart, windowEnd: config.executionWindowEnd, waitMs: jitterMs, jitterMs, reason: "JITTER" };
  }

  return { windowEnabled: config.scheduleWindowEnabled, windowStart: config.executionWindowStart, windowEnd: config.executionWindowEnd, waitMs: 0, jitterMs: 0, reason: "NONE" };
}
