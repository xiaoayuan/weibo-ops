...
export function extractStatusIdFromUrl(targetUrl: string) {
  ...
  for (const candidate of candidates) {
    ...
    if (longDigits && longDigits.length > 0) return longDigits[0];
  }
...
function collectCandidatePosts(payload: unknown, fallbackTopicUrl: string): CandidatePost[] {
  ...
  while (queue.length > 0) {
    ...
    if (idRaw) {
      ...
      if (!seen.has(id)) {
        seen.add(id);
        const userId = String((record.user && typeof record.user === "object" ? (record.user as Record<string, unknown>).idstr || (record.user as Record<string, unknown>).id : "") || "");
    ...
    for (const value of Object.values(record)) {
      if (value && (Array.isArray(value) || typeof value === "object")) queue.push(value);
    }
  }
...
function getTimelineEndpoints(topicUrl: string, containerId?: string) {
  const configured = process.env.WEIBO_FIRST_COMMENT_TIMELINE_ENDPOINTS?.split(",").map((item) => item.trim()).filter(Boolean);
  const templates = configured && configured.length > 0
    ? configured
    : [
  ...
  return templates.map((template) => template.replaceAll("{containerid}", encodeURIComponent(containerId || "")).replaceAll("{topic_url_enc}", encodeURIComponent(topicUrl)));
}

function collectStatusIdsFromTopicHtml(html: string) {
  const ids: string[] = [];
  const patterns = [/"id":"(\d{15,20})"/g, /"mid":"(\d{15,20})"/g, /\bmid=(\d{15,20})\b/g];
  for (const pattern of patterns) {
    for (const matched of html.matchAll(pattern)) {
      if (matched[1]) ids.push(matched[1]);
    }
  }
...
async function fetchLatestPostsFromTopicPage(topicUrl: string, cookie: string, limit: number, proxyConfig?: ProxyConfig | null) {
  ...
  if (!response.ok) return [] as CandidatePost[];
...
function isCommentCreated(payload: unknown) {
  ...
  if (typeof record.id === "number" || (typeof record.id === "string" && record.id.trim())) return true;
  if (typeof record.idstr === "string" && record.idstr.trim()) return true;
  if (record.data && typeof record.data === "object") return isCommentCreated(record.data);
...
export async function fetchStatusCommentsCount(statusId: string, cookie: string, referer: string, proxyConfig?: ProxyConfig | null) {
  ...
  if (!response.ok || !payload || typeof payload !== "object") return undefined;
  const record = payload as Record<string, unknown>;
...
export async function checkStatusIsZeroComments(statusId: string, cookie: string, referer: string, fallbackCount?: number, proxyConfig?: ProxyConfig | null) {
  if (typeof fallbackCount === "number") return fallbackCount === 0;
