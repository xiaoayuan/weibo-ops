...
import { classifyExecutionOutcome, type ExecutionErrorClass } from "@/src/lib/error-classifier";
...
function computeRiskDelta(errorClass: ExecutionErrorClass, rules: Awaited<ReturnType<typeof getRiskRules>>) {
  if (errorClass === "ACCOUNT_RISK") return rules.score.accountRisk;
  if (errorClass === "TRANSIENT_NETWORK") return rules.score.transientNetwork;
  if (errorClass === "PLATFORM_BUSY") return rules.score.platformBusy;
  if (errorClass === "TARGET_ISSUE") return rules.score.targetIssue;
  if (errorClass === "CONTENT_ISSUE") return rules.score.contentIssue;
  if (errorClass === "SUCCESS") return rules.score.success;
...
export async function classifyAndApplyAccountRisk(input: RiskInput): Promise<RiskMeta> {
  ...
  if (!input.accountId) {
    return { errorClass, riskDelta, consecutiveFailureDelta: failureDelta };
  }

  const account = await prisma.weiboAccount.findUnique({
    where: { id: input.accountId },
    select: { id: true, status: true, riskLevel: true, consecutiveFailures: true },
  });

  if (!account) {
    return { errorClass, riskDelta: 0, consecutiveFailureDelta: 0 };
  }
