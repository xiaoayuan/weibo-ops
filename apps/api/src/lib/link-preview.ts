import { sendHttpRequest } from "@/src/lib/http-client";
...
function extractContentFromHtml(html: string) {
  ...
  const cleaned = candidates.map((item) => item.replace(/^微博正文[:：]?/, "").trim()).filter((item) => item.length >= 6);
  return cleaned[0] || null;
}

function suggestBusinessType(content: string): SuggestedBusinessType {
  if (/抽奖|转发|带话题|扩散|控评|评论区|统一口径/i.test(content)) return "COMMENT_CONTROL";
  if (/转发|扩|帮转|轮转/i.test(content)) return "REPOST_ROTATION";
  if (/求问|怎么看|有没有|有人知道|为什么|如何|吗[？?]?$/i.test(content)) return "QUICK_REPLY";
  return "DAILY_PLAN";
}

function suggestTone(content: string): SuggestedTone {
  if (/哈哈|hh|笑死|可爱|啊啊|呜呜|太绝了/i.test(content)) return "LIVELY";
  if (/支持|冲|加油|喜欢|好看|真不错/i.test(content)) return "SUPPORTIVE";
  if (/我觉得|感觉|怎么看|讨论|其实/i.test(content)) return "DISCUSSIVE";
  if (/路过|围观|吃瓜|看到/i.test(content)) return "PASSERBY";
  return "NATURAL";
}
