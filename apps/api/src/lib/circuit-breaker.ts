...
import type { ExecutionErrorClass } from "@/src/lib/error-classifier";
import { getExecutionStrategy } from "@/src/lib/strategy-config";
...
async function getSettingValue<T>(key: string): Promise<T | undefined> {
  ...
  if (!setting?.value || typeof setting.value !== "object") return undefined;
...
export async function isProxyCircuitOpen(proxyNodeId?: string | null) {
  if (!proxyNodeId) return false;
  const state = await getSettingValue<ProxyCircuitState>(proxyKey(proxyNodeId));
  const pausedUntil = parseDate(state?.pausedUntil);
  return Boolean(pausedUntil && pausedUntil.getTime() > Date.now());
}

export async function recordExecutionOutcome(input: { accountId?: string | null; proxyNodeId?: string | null; success: boolean; errorClass?: ExecutionErrorClass }) {
  ...
  if (input.accountId) {
    ...
    if (input.success || accountFailureRelevant) {
      ...
      const nextPausedUntil = consecutiveFailures >= accountFailureThreshold ? new Date(now + accountPauseMs).toISOString() : pausedActive ? pausedUntil?.toISOString() : undefined;

      await upsertSettingValue(key, { consecutiveFailures, pausedUntil: nextPausedUntil });
    }
  }
  ...
  if (input.proxyNodeId) {
    ...
    if (input.success || proxyFailureRelevant) {
      ...
      const nextPausedUntil = total >= proxyMinSamples && failureRatio > proxyFailureRatio ? new Date(now + proxyPauseMs).toISOString() : pausedActive ? pausedUntil?.toISOString() : undefined;
