import { getAiRiskConfig } from "@/src/lib/ai-risk-config";
import { resolveAiRuntimeConfig } from "@/src/lib/ai-copywriting-config";
...
async function requestAiRisk(prompt: string) {
  ...
  const response = await fetch(baseUrl, {
    ...
    body: JSON.stringify({
      ...
      temperature: 0.2,
      messages: [
        { role: "system", content: "你是微博运营风控助手。请只输出 JSON，格式为 { riskLevel, summary, reasons, suggestions }，其中 riskLevel 只能是 LOW、MEDIUM、HIGH。" },
  ...
  return JSON.parse(stripCodeFences(content)) as {
    ...
    reasons?: string[];
    suggestions?: string[];
    items?: Partial<AiRiskAssessment>[];
  };
}
...
async function assessCopywritingFallback(content: string, peerContents: string[], canBlock = true): Promise<AiRiskAssessment> {
  ...
  return normalizeAssessment({ riskLevel, summary: reasons.length > 0 ? reasons[0] : "文案整体较自然", reasons, suggestions }, canBlock);
}

export async function assessCopywritingCandidates(input: { businessType: string; context: string; candidates: string[] }) {
  const prompt = [
...
export async function assessTaskRisk(input: { taskType: string; urgency: string; accountCount: number; context?: string }) {
  const prompt = [
    `任务类型：${input.taskType}`,
    `时效等级：${input.urgency}`,
...
export async function summarizeFailureRisk(input: { actionText: string; detailText: string; topReason?: string | null }) {
  const prompt = [
    `动作：${input.actionText}`,
    `详情：${input.detailText}`,
