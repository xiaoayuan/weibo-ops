...
export const defaultExecutionStrategy: ExecutionStrategy = {
  actionJob: {
    ...
    urgency: {
      S: {
        waveRatios: [0.4, 0.3, 0.3],
        waveWindowsSec: [2, 10, 30],
        cooldownSecRange: [2, 8],
        retryDelaySecRange: [1, 3],
        targetSlaSec: 240,
        limitSlaSec: 480,
      },
      A: {
...
function normalizeStrategy(strategy: ExecutionStrategy): ExecutionStrategy {
  const normalizeUrgency = (item: ExecutionStrategy["actionJob"]["urgency"]["S"]) => {
    ...
    return {
      ...
      waveWindowsSec: [
        item.waveWindowsSec[0],
        Math.max(item.waveWindowsSec[0], item.waveWindowsSec[1]),
        Math.max(item.waveWindowsSec[1], item.waveWindowsSec[2]),
      ] as [number, number, number],
      cooldownSecRange: [
        Math.min(item.cooldownSecRange[0], item.cooldownSecRange[1]),
        Math.max(item.cooldownSecRange[0], item.cooldownSecRange[1]),
      ] as [number, number],
      retryDelaySecRange: [
        Math.min(item.retryDelaySecRange[0], item.retryDelaySecRange[1]),
        Math.max(item.retryDelaySecRange[0], item.retryDelaySecRange[1]),
      ] as [number, number],
